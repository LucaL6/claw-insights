...
import { buildSnapshotData, normalize, relativeTime, type DataSources } from '../data-service';
...
function makeSources(sessionCount = 3): DataSources {
  const sessions = Array.from({ length: sessionCount }, (_, i) => ({
    ...
    subAgents: [
      { displayName: `Sub Agent ${i}A`, name: `sub-${i}-a`, status: 'running', completed: false, updatedAt: new Date().toISOString() },
    ],
  }));
  ...
  return {
    ...
    getChannels: async () => [
      { provider: 'discord', name: 'main', connected: true, latencyMs: 42 },
    ],
...
describe('normalize', () => {
  test('normalizes values to 0-100', async () => {
...
describe('relativeTime', () => {
  test('should handle epoch ms number', async () => {
    const fiveMinAgo = Date.now() - 5 * 60_000;
    expect(relativeTime(fiveMinAgo)).toBe('5m ago');
  });

  test('should handle ISO string', async () => {
    const fiveMinAgo = new Date(Date.now() - 5 * 60_000).toISOString();
    expect(relativeTime(fiveMinAgo)).toBe('5m ago');
  });

  test('should handle 0 gracefully', async () => {
    const result = relativeTime(0);
    expect(result).toBe('—');
  });

  test('should return — for NaN', async () => {
