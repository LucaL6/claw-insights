...
function insertHourlySample(db: Database, hour: string, opts: {
  sessionsMax?: number; sessionsAvg?: number; tokenDelta?: number;
  costEnd?: number; cpuAvg?: number; cpuMax?: number;
  memAvg?: number; memMax?: number; count?: number;
}) {
  db.prepare(`INSERT INTO hourly_metric_samples
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
    hour,
    opts.sessionsMax ?? 0, opts.sessionsAvg ?? 0, opts.tokenDelta ?? 0,
    opts.costEnd ?? 0, opts.cpuAvg ?? 0, opts.cpuMax ?? 0,
    opts.memAvg ?? 0, opts.memMax ?? 0, opts.count ?? 60,
  );
}

function insertHourlyModel(db: Database, hour: string, model: string, tokenDelta: number) {
  db.prepare('INSERT INTO hourly_model_tokens (hour, model, token_delta_k) VALUES (?, ?, ?)').run(hour, model, tokenDelta);
}
...
describe('Query routing — hourly fallback', () => {
  ...
  it('should use raw table when useHourly=false (default)', () => {
    db.prepare(`INSERT INTO metric_samples (timestamp, active_sessions, total_tokens_k, token_delta_k, cost_today, tokens_today_m, cpu, memory_mb) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
