...
afterEach(() => {
  ...
  if (savedEnv.CLAW_INSIGHTS_CLI !== undefined) {process.env.CLAW_INSIGHTS_CLI = savedEnv.CLAW_INSIGHTS_CLI;}
  else {delete process.env.CLAW_INSIGHTS_CLI;}
  if (savedEnv.OPENCLAW_CLI !== undefined) {process.env.OPENCLAW_CLI = savedEnv.OPENCLAW_CLI;}
  else {delete process.env.OPENCLAW_CLI;}
  try {
