...
export const createSourceResolvers = (ctx: AppContext): Partial<Resolvers> => {
  const registry = createSourceRegistry();

  // Static dashboard source (no business logic — info-only namespace)
  const dashboardSource: SourceAdapter = {
    info: {
      id: 'dashboard:main',
      name: 'Claw Insights Dashboard',
      status: 'CONNECTED',
      attributes: {
        category: 'DASHBOARD',
        provider: 'OPENCLAW',
  ...
  registry.register(dashboardSource);
  ...
  const query = {
    ...
    source: (
      ...
      // Dashboard sources return info-only namespace
      if (found.info.attributes.category === 'DASHBOARD') {
        return { _info: found.info };
      }
  ...
  const agentNamespace = {
    ...
    session: async (_parent: unknown, args: { key: string }) => {
      ...
      return safe(() => Promise.resolve(ctx.ports.sessions.getSessionById(args.key, readCtx)));
    },
  ...
  const dashboardNamespace = {
    info: (parent: unknown) => {
      const p = parent as { _info: ReturnType<typeof registry.list>[number] };
      return p._info;
    },
  };
