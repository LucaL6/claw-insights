...
const LEVEL_STYLES: Record<string, { bg: string; color: string; border: string; label: string; gutter: string }> = {
  error: { bg: 'var(--red-bg)', color: 'var(--red)', border: 'var(--red-border)', label: 'ERROR', gutter: 'var(--red)' },
  warning: { bg: 'var(--amber-bg)', color: 'var(--amber)', border: 'var(--amber-border)', label: 'WARN', gutter: 'var(--amber)' },
  gateway_restart: { bg: 'var(--orange-bg)', color: 'var(--orange)', border: 'var(--orange-border)', label: 'RESTART', gutter: 'var(--orange)' },
};
...
export function EventRow({ timestamp, type, module, message, highlighted }: EventRowProps) {
  ...
  return (
    ...
    >
      ...
      {/* Module pill */}
      <span
        className="mono text-[10px] px-2 py-0.5 rounded truncate text-center"
        style={{ backgroundColor: 'var(--bg-elevated)', color: 'var(--text-dim)', border: '1px solid var(--border-subtle)' }}
      >
        {module}
      </span>
      {/* Message */}
