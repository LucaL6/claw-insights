...
|----------|---------|-------------|
| `CLAW_INSIGHTS_SERVER_PORT` | `4000` | GraphQL API + web server port |
| `CLAW_INSIGHTS_WEB_PORT` | `3200` | Vite dev server port (development only) |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `CLAW_INSIGHTS_FONTS_DIR` | *(bundled)* | Custom font directory for Satori renderer |
...
| Server port | 4000 | 4111 | 4000 |
| Web port | 3200 | 3211 | 3200 |
...
2. Prints token URL: `🔑 http://localhost:4000/?token=xxx`
...
curl -H "Authorization: Bearer YOUR_EXAMPLE_TOKEN # gitleaks:allow" http://localhost:4000/graphql \
